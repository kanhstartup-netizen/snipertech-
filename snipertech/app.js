...
function SniperTechX() {
    ...
    const analyze = async () => {
        ...
        const autoCancel = setTimeout(() => { setLoading(false); setStage(""); setErr("ໃຊ້ເວລາດົນເກີນ (90 ວິ). ກວດ internet ແລ້ວລອງໃໝ່."); }, 90000);
      ...
      `),
            ...
            nav === "home" && (React.createElement("div", { className: "fx-rise" },
                ...
                React.createElement("div", { style: { marginTop: 24 } },
                    ...
                    React.createElement("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginTop: 14 } },
                        React.createElement(HomeCard, { icon: "\uD83D\uDCCA", title: t("analyzeChart"), desc: "AI Intelligence", glowColor: "rgba(38,130,255,.2)", iconBg: "rgba(38,130,255,.15)", onClick: () => { setLoading(false); setStage(""); setNav("tools"); setTab("chart"); } }),
                        React.createElement(HomeCard, { icon: "\uD83D\uDCF0", title: t("analyzeNews"), desc: "DXY \u00B7 Fed \u00B7 Oil", glowColor: "rgba(63,217,138,.15)", iconBg: "rgba(63,217,138,.12)", onClick: () => { setLoading(false); setStage(""); setNav("tools"); setTab("news"); } }),
        ...
        React.createElement("nav", { style: { position: "fixed", left: "50%", bottom: 0, transform: "translateX(-50%)", width: "100%", maxWidth: 720, zIndex: 50, padding: "0 12px 12px" } },
            React.createElement("div", { style: { display: "flex", justifyContent: "space-around", alignItems: "center", gap: 4, padding: "8px 6px", borderRadius: 20, border: `1px solid ${C.line}`, background: "rgba(16,20,30,.82)", backdropFilter: "blur(18px)", WebkitBackdropFilter: "blur(18px)", boxShadow: "0 -8px 30px -12px rgba(0,0,0,.7)" } }, [
                ...
                return (React.createElement("button", { key: item.id, onClick: () => { setLoading(false); setStage(""); setNav(item.id); window.scrollTo({ top: 0, behavior: "smooth" }); }, style: { flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 3, padding: "6px 2px", border: "none", background: "transparent", cursor: "pointer", fontFamily: "inherit", position: "relative" } },
