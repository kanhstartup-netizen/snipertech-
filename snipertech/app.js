...
function getAccount(email) { const k = emailKey(email); if (!k) return null; return loadAccounts()[k] || null; }
function saveAccount(rec) {
    const k = emailKey(rec && rec.email); if (!k) return rec;
    try { const all = loadAccounts(); all[k] = { ...(all[k] || {}), ...rec, email: rec.email }; localStorage.setItem(ACCT_KEY, JSON.stringify(all)); return all[k]; }
    catch(e) { return rec; }
}

// ── Cloud membership sync (cross-device VIP via Google Sheets) ──
// Stable per-device id. Persists in localStorage so the same browser/device
// keeps the same id. Used to BIND a paid membership to ONE device, so a VIP
// or Life Time account can't silently run on multiple devices at once.
const DEVICE_KEY = "sniper_device_id";
...
async function getMemberCloud(email) {
    ...
    try {
        ...
        if (d && d.found && d.expiresAt) return {
            plan: d.plan || "VIP",
            expiresAt: Number(d.expiresAt),
            // deviceId that the membership is bound to (empty/undefined = not yet bound)
            deviceId: d.deviceId || d.device_id || ""
        };
        return null;
    } catch(e) { return null; }
}
function setMemberCloud(email, plan, expiresAt, deviceId) {
    if (!SHEETS_URL || !email) return;
    try {
        let url = `${SHEETS_URL}?action=setMember&email=${encodeURIComponent(emailKey(email))}&plan=${encodeURIComponent(plan)}&expiresAt=${encodeURIComponent(expiresAt)}`;
...
const FREE_LESSONS = [
    ...
    },
    {
        ...
        lo: {
            ...
            sections: [
                ...
                { h: "ວິທີຕັ້ງ TP ແບບມືອາຊີບ", p: "ຕັ້ງ TP ທີ່ Liquidity pool ຕໍ່ໄປ ບໍ່ແມ່ນເລກມົ້ວໆ. ໃຊ້ປິດເປັນຊັ້ນ: TP1 ປິດ ⅓-½ ແລ້ວຍ້າຍ SL ມາ break-even, TP2 ກາງ, TP3 ໄກ + ເຫຼືອ runner ນ້ອຍ. ໃຫ້ TP1 ໄດ້ RR ຢ່າງໜ້ອຍ 1:2." },
                { h: "ວິໄນຄວາມສ່ຽງແບບ Prop Firm", p: "ສ່ຽງພຽງ 0.5-1% ຕໍ່ໄມ້, ບໍ່ averaging/martingale, ຢຸດເມື່ອເສຍ 2-3 ໄມ້ຕິດ, ເຄົາລົບ daily loss limit. ໄມ້ A+ ໜຶ່ງ ດີກວ່າ ໄມ້ທຳມະດາ 10 ໄມ້. ‘ບໍ່ຜິດດົນ’ ສຳຄັນກວ່າ ‘ຖືກ’." },
            ],
            highlights: [
                "ລໍ Sweep + BOS ກ່ອນເຂົ້າ ສະເໝີ.",
                "Sell ຈາກ Premium, Buy ຈາກ Discount ເທົ່ານັ້ນ.",
                "TP ທີ່ Liquidity ຕໍ່ໄປ + ປິດເປັນຊັ້ນ.",
                "ສ່ຽງ 0.5-1% — ຊະນະ 40% ທີ່ RR 1:3 ກໍກຳໄລ.",
            ],
            summary: "ສະຖາບັນ ‘ສ້າງ’ ການເຄື່ອນໄຫວ ບໍ່ໄລ່ຕາມ. ລໍລະດັບ → Sweep → ຢືນຢັນ → ເຂົ້າ → ໄປ Liquidity ຕໍ່ໄປ ພ້ອມວິໄນຄວາມສ່ຽງເຂັ້ມ ຄືກະແຈສຳຄັນ.",
        },
        th: {
    ...
    },
    {
        ...
        lo: {
            ...
            sections: [
                ...
                { h: "ໃຊ້ຢູ່ໃສ", p: "ສັນຍານແທ່ງທຽນມີນ້ຳໜັກທີ່ສຸດເມື່ອເກີດຢູ່ ໂຊນສຳຄັນ (OB, supply/demand, ແນວຮັບ-ແນວຕ້ານ) ບໍ່ແມ່ນກາງອາກາດ." },
            ],
            highlights: ["ໂຕຍາວ = ແຮງ, ໄສ້ຍາວ = ຖືກປະຕິເສດ.", "Pin Bar / Engulfing = ສັນຍານກັບໂຕເດັ່ນ.", "ເບິ່ງແທ່ງທີ່ໂຊນສຳຄັນເທົ່ານັ້ນ."],
            summary: "ແທ່ງທຽນຄືພາສາຂອງຕະຫຼາດ. ເບິ່ງ body/wick + ຕຳແໜ່ງທີ່ເກີດ ແລ້ວຈະອ່ານແຮງຊື້ແຮງຂາຍອອກ.",
        },
        th: {
    ...
    },
    {
        ...
        lo: {
            title: "Supply & Demand (ໂຊນອຸປະທານ-ອຸປະສົງ)",
            minutes: 7,
            intro: "ໂຊນ Supply/Demand ຄືບ່ອນທີ່ລາຄາເຄີຍພຸ່ງແຮງ — ເປັນຮ່ອງຮອຍຂອງຄຳສັ່ງໃຫຍ່ທີ່ຍັງເຫຼືອຢູ່.",
            sections: [
                { h: "Demand zone (ແຮງຊື້)", p: "ບໍລິເວນທີ່ລາຄາເຄີຍເດັ້ງຂຶ້ນແຮງ. ເມື່ອລາຄາກັບລົງມາແຕະ ມັກມີແຮງຊື້ລໍຢູ່ → ຫາຈັງຫວະ Buy." },
                { h: "Supply zone (ແຮງຂາຍ)", p: "ບໍລິເວນທີ່ລາຄາເຄີຍຮ່ວງແຮງ. ເມື່ອລາຄາຂຶ້ນມາແຕະ ມັກມີແຮງຂາຍລໍຢູ່ → ຫາຈັງຫວະ Sell." },
                { h: "ໂຊນສົດ vs ໂຊນໃຊ້ແລ້ວ", p: "ໂຊນທີ່ຍັງບໍ່ຖືກແຕະ (fresh) ແຮງທີ່ສຸດ. ຍິ່ງຖືກແຕະຫຼາຍຄັ້ງ ຍິ່ງອ່ອນລົງ ແລະ ມີໂອກາດທະລຸ." },
            ],
            highlights: ["Demand = ແຮງຊື້ (Buy), Supply = ແຮງຂາຍ (Sell).", "ໂຊນສົດ (fresh) ແຮງທີ່ສຸດ.", "ລວມກັບ candle ຢືນຢັນກ່ອນເຂົ້າ."],
            summary: "Supply/Demand ໃຫ້ກອບວ່າຈະ Buy/Sell ຢູ່ໃສ. ລໍລາຄາມາທີ່ໂຊນສົດ + ແທ່ງຢືນຢັນ ແລ້ວຄ່ອຍເຂົ້າ.",
        },
        th: {
    ...
    },
    {
        ...
        lo: {
            title: "ເສັ້ນແນວໂນ້ມ & ແນວຮັບແນວຕ້ານ",
            minutes: 5,
            intro: "ເຄື່ອງມືພື້ນຖານທີ່ມີພະລັງ: ເສັ້ນແນວໂນ້ມ (trendline) ແລະ ແນວຮັບ-ແນວຕ້ານ ຊ່ວຍບອກທິດ ແລະ ຈຸດກັບໂຕ.",
            sections: [
                { h: "ເສັ້ນແນວໂນ້ມ (Trendline)", p: "ລາກເຊື່ອມຈຸດຕ່ຳທີ່ສູງຂຶ້ນ (ຂາຂຶ້ນ) ຫຼື ຈຸດສູງທີ່ຕ່ຳລົງ (ຂາລົງ). ຍິ່ງລາຄາແຕະເສັ້ນຫຼາຍຄັ້ງໂດຍບໍ່ທະລຸ ເສັ້ນຍິ່ງແຂງແຮງ." },
                { h: "ແນວຮັບ-ແນວຕ້ານ", p: "ແນວຮັບ = ລາຄາທີ່ມັກເດັ້ງຂຶ້ນ, ແນວຕ້ານ = ລາຄາທີ່ມັກຖືກກົດ. ເມື່ອທະລຸແນວຕ້ານ ມັນມັກກາຍເປັນແນວຮັບໃໝ່ (ແລະ ກັບກັນ)." },
                { h: "ການທະລຸ (Breakout)", p: "ລໍ ແທ່ງປິດ ທະລຸຈິງ ບໍ່ແມ່ນພຽງໄສ້. ລະວັງ fake breakout — ລໍ retest ເສັ້ນເກົ່າກ່ອນເຂົ້າຈະປອດໄພກວ່າ." },
            ],
            highlights: ["ເສັ້ນທີ່ຖືກແຕະຫຼາຍຄັ້ງ = ແຂງແຮງ.", "ແນວຕ້ານທີ່ທະລຸ → ກາຍເປັນແນວຮັບ.", "ລໍ retest ຫຼັງ breakout ປອດໄພກວ່າ."],
            summary: "ເສັ້ນແນວໂນ້ມ ແລະ ແນວຮັບຕ້ານ ໃຫ້ໂຄງສ້າງງ່າຍໆ ທີ່ໃຊ້ໄດ້ຈິງ. ລວມກັບ price action ເພື່ອຈັງຫວະເຂົ້າທີ່ດີ.",
        },
        th: {
    ...
    },
    {
        ...
        lo: {
            title: "Fibonacci ໃຊ້ຈິງແບບມືອາຊີບ",
            minutes: 6,
            intro: "Fibonacci ບໍ່ແມ່ນເວດມົນ ແຕ່ເປັນແຜນທີ່ຂອງ ລາຄາທີ່ຕະຫຼາດມັກກັບມາ. ໃຊ້ຄູ່ກັບ structure ຈະແມ່ນຂຶ້ນຫຼາຍ.",
            sections: [
                { h: "ລະດັບສຳຄັນ", p: "0.5 ແລະ 0.618 = ຈຸດຍໍ່ໂຕທີ່ຕະຫຼາດມັກກັບໂຕ. 0.705-0.79 = ໂຊນ OTE (ເຂົ້າດີສຸດ). ສ່ວນ 0 ແລະ 1 ຄືຕົ້ນແລະປາຍຄື້ນ." },
                { h: "ວິທີລາກທີ່ຖືກ", p: "ລາກຈາກ swing low → swing high (ຂາຂຶ້ນ) ຫຼື ກັບກັນ. ໃຊ້ຄື້ນທີ່ ສະອາດ ຊັດເຈນ ບໍ່ແມ່ນລາກມົ້ວ." },
                { h: "ລວມກັບ Confluence", p: "Fib ຈະແມ່ນທີ່ສຸດເມື່ອລະດັບມັນທັບກັບ OB / FVG / ແນວຮັບຕ້ານ — ນັ້ນຄືຈຸດເຂົ້າຄຸນນະພາບສູງ." },
            ],
            highlights: ["0.618 ແລະ OTE 0.705-0.79 = ໂຊນເຂົ້າເດັ່ນ.", "ລາກຈາກ swing ທີ່ສະອາດເທົ່ານັ້ນ.", "Fib ທັບ OB/FVG = ຈຸດເຂົ້າຄຸນນະພາບ."],
            summary: "Fibonacci ຊ່ວຍຫາ ລາຄາທີ່ດີ ໃນການເຂົ້າ. ໃຊ້ຄູ່ structure + OB ສະເໝີ ຢ່າໃຊ້ດ່ຽວໆ.",
        },
        th: {
    ...
    },
    {
        ...
        lo: {
            ...
            minutes: 6,
            intro: "ມືອາຊີບບໍ່ເບິ່ງ TF ດຽວ. ເຂົາໄລ່ຈາກໃຫຍ່ໄປນ້ອຍ (top-down) ເພື່ອເທຣດຕາມທິດເງິນໃຫຍ່ ແລະ ເຂົ້າໃຫ້ແມ່ນ.",
            sections: [
                { h: "TF ໃຫຍ່ = ທິດທາງ", p: "D1/H4 ບອກ bias ຫຼັກ (ຂຶ້ນ/ລົງ) ແລະ ໂຊນສຳຄັນ (HTF OB, supply/demand). ຫ້າມສວນທິດນີ້." },
                { h: "TF ກາງ = ຈັງຫວະ", p: "H1/M15 ຫາວ່າລາຄາກຳລັງເຂົ້າໃກ້ໂຊນຂອງ TF ໃຫຍ່ບໍ ແລະ ເລີ່ມມີ sweep/CHoCH ຫຼືຍັງ." },
                { h: "TF ນ້ອຍ = ຈຸດເຂົ້າ", p: "M5/M1 ໃຊ້ຫາ entry ທີ່ແມ່ນ (OB/FVG ນ້ອຍ + ແທ່ງຢືນຢັນ) ເຮັດໃຫ້ SL ສັ້ນແຕ່ທິດຕົງກັບເງິນໃຫຍ່." },
            ],
            highlights: ["ໃຫຍ່ບອກທິດ, ກາງບອກຈັງຫວະ, ນ້ອຍບອກຈຸດເຂົ້າ.", "ຫ້າມສວນ bias ຂອງ TF ໃຫຍ່.", "ເຂົ້າ TF ນ້ອຍ = SL ສັ້ນ RR ດີ."],
            summary: "Top-down ຄືຫົວໃຈຂອງການເທຣດສະຖາບັນ: ເທຣດຕາມທິດ HTF, ເຂົ້າແມ່ນທີ່ LTF. RR ຈະດີຂຶ້ນຊັດເຈນ.",
        },
        th: {
    ...
    },
    {
        ...
        lo: {
            ...
            minutes: 5,
            intro: "ທອງຄຳເຄື່ອນໄຫວແຮງສະເພາະບາງຊ່ວງເວລາ. ຮູ້ ເວລາທອງ ຈະເທຣດໜ້ອຍລົງແຕ່ໄດ້ຫຼາຍຂຶ້ນ.",
            sections: [
                { h: "London (ບ່າຍ)", p: "ຊ່ວງເປີດລອນດອນ ເປັນຊ່ວງທີ່ທອງແລ່ນແຮງ ມີ sweep ແລະ move ຫຼັກຂອງມື້ເລື້ອຍໆ." },
                { h: "New York (ກາງຄືນ)", p: "ເປີດ NY ທັບກັບລອນດອນ = volume ສູງສຸດ. ຂ່າວສະຫະລັດສ່ວນຫຼາຍອອກຊ່ວງນີ້ ລະວັງຄວາມຜັນຜວນ." },
                { h: "Asian (ເຊົ້າ-ສາຍ)", p: "ຊ່ວງເອເຊຍມັກນິ່ງ ສ້າງ range ແຄບ ໃຊ້ດັກຫາ liquidity ໄວ້ໃຫ້ London/NY ມາກວາດ." },
            ],
            highlights: ["London & NY open = ເວລາທອງຂອງທອງ.", "Asian range = ແຫຼ່ງ liquidity.", "ຫຼີກລ່ຽງເທຣດຊ່ວງຕະຫຼາດງຽບ."],
            summary: "ເທຣດຖືກເວລາ = ເຄິ່ງໜຶ່ງຂອງຄວາມສຳເລັດ. ເນັ້ນ killzone London/NY ແລ້ວຄຸນນະພາບສັນຍານຈະສູງຂຶ້ນ.",
        },
        th: {
    ...
    },
    {
        ...
        lo: {
            title: "ຈິດຕະວິທະຍາການເທຣດ (Mindset)",
            minutes: 7,
            intro: "ລະບົບດີແຄ່ໃດກໍ່ໄຮ້ຄ່າຖ້າໃຈບໍ່ນິ່ງ. 80% ຂອງຄວາມສຳເລັດຄືຈິດຕະວິທະຍາ ບໍ່ແມ່ນກົນລະຍຸດ.",
            sections: [
                { h: "ສັດຕູໂຕຈິງ: ອາລົມ", p: "ຄວາມໂລບ (ເພີ່ມ lot, ບໍ່ TP), ຄວາມຢ້ານ (ບໍ່ກ້າເຂົ້າ, ປິດໄວ), revenge trade ຄືສາເຫດທີ່ port ພັງ ບໍ່ແມ່ນກຣາຟ." },
                { h: "ວິໄນ = ອິດສະຫຼະພາບ", p: "ມີແຜນກ່ອນເຂົ້າສະເໝີ (entry/SL/TP) ແລ້ວເຮັດຕາມແຜນ ບໍ່ປ່ຽນກາງຄັນ. ຍອມຮັບການຂາດທຶນເປັນຕົ້ນທຶນທຸລະກິດ." },
                { h: "ເທຣດເໝືອນນັກທຸລະກິດ", p: "ຄິດເປັນຄວາມໜ້າຈະເປັນໃນໄລຍະຍາວ. ໄມ້ດຽວບໍ່ສຳຄັນ ສຳຄັນທີ່ເຮັດຊ້ຳລະບົບທີ່ມີ edge ໄດ້ຢ່າງນິ່ງໆ 100 ໄມ້." },
            ],
            highlights: ["80% ຄືຈິດຕະວິທະຍາ ບໍ່ແມ່ນກົນລະຍຸດ.", "ຫ້າມ revenge trade ເດັດຂາດ.", "ເຮັດຕາມແຜນ = ອິດສະຫຼະພາບ."],
            summary: "ຕະຫຼາດຈ່າຍເງິນໃຫ້ຄົນໃຈນິ່ງແລະມີວິໄນ. ຄວບຄຸມອາລົມໄດ້ = ຄວບຄຸມ port ໄດ້ ນີ້ຄືທັກສະທີ່ແພງທີ່ສຸດ.",
        },
        th: {
    ...
    },
    {
        ...
        lo: {
            title: "ເທຣດຂ່າວ & DXY ກັບທອງຄຳ",
            minutes: 6,
            intro: "ທອງຄຳອ່ອນໄຫວຕໍ່ຂ່າວສະຫະລັດ ແລະ ຄ່າເງິນໂດລາຫຼາຍ. ເຂົ້າໃຈຄວາມສຳພັນນີ້ = ໄດ້ປຽບ ແລະ ຫຼີກລ່ຽງໂດນກວາດ.",
            sections: [
                { h: "DXY ↔ ທອງ (ສວນທາງ)", p: "ໂດລາຂຶ້ນ (DXY up) → ທອງມັກລົງ. ໂດລາລົງ → ທອງມັກຂຶ້ນ. ເຊັກ DXY ກ່ອນເຂົ້າສະເໝີ ຖ້າສວນທາງກັບແຜນ ໃຫ້ລະວັງ." },
                { h: "ຂ່າວແຮງທີ່ຕ້ອງຮູ້", p: "FOMC, ດອກເບ້ຍ Fed, NFP (ການຈ້າງງານ), CPI/PCE (ເງິນເຟີ້) — ຂ່າວພວກນີ້ເຮັດໃຫ້ທອງແລ່ນແຮງຫຼາຍ ແລະ ຄາດເດົາຍາກ." },
                { h: "ກົນລະຍຸດຊ່ວງຂ່າວ", p: "ມືໃໝ່: ຢ່າເທຣດກ່ອນ-ລະຫວ່າງຂ່າວແຮງ (spread ກວ້າງ ໂດນກວາດງ່າຍ). ມືອາຊີບ: ລໍຂ່າວອອກ ລໍ sweep + ໂຄງສ້າງນິ່ງ ແລ້ວຄ່ອຍຕາມ." },
            ],
            highlights: ["DXY ຂຶ້ນ = ທອງລົງ (ສວນທາງ).", "FOMC/NFP/CPI = ລະວັງສຸດ.", "ມືໃໝ່ຫຼີກລ່ຽງເທຣດຕອນຂ່າວແຮງ."],
            summary: "ຂ່າວ ແລະ DXY ຄືພະລັງເບື້ອງຫຼັງທອງ. ຢ່າເທຣດຕາບອດ ເຊັກປະຕິທິນຂ່າວ + ທິດ DXY ກ່ອນສະເໝີ.",
        },
        th: {
    ...
    },
    {
        ...
        lo: {
            title: "ສ້າງແຜນເທຣດ & ຈົດບັນທຶກ (Journal)",
            minutes: 6,
            intro: "ເທຣດເດີ້ທີ່ກຳໄລສະໝ່ຳສະເໝີ ມີແຜນຊັດ ແລະ ຈົດທຸກໄມ້ ບໍ່ແມ່ນເທຣດຕາມອາລົມ. ນີ້ຄືສິ່ງທີ່ແຍກມືໂປຣກັບມືສະໝັກຫຼິ້ນ.",
            sections: [
                { h: "ແຜນເທຣດ (Trading Plan)", p: "ກຳນົດລ່ວງໜ້າ: ເທຣດຄູ່ໃດ, TF ໃດ, setup ແບບໃດທີ່ຈະເຂົ້າ, ຄວາມສ່ຽງຕໍ່ໄມ້, ເວລາທີ່ເທຣດ. ຖ້າບໍ່ຕົງເງື່ອນໄຂ = ບໍ່ເຂົ້າ." },
                { h: "ຈົດບັນທຶກ (Journal)", p: "ທຸກໄມ້ຈົດ: ເຫດຜົນເຂົ້າ, ພາບກຣາຟ, ຜົນລັບ, ອາລົມຕອນນັ້ນ. ທົບທວນທຸກອາທິດເພື່ອຫາຈຸດທີ່ເຮັດຜິດຊ້ຳ." },
                { h: "ວັດຜົນດ້ວຍຕົວເລກ", p: "ເບິ່ງ win rate, RR ສະເລ່ຍ, ໄມ້ທີ່ດີທີ່ສຸດ/ແຍ່ທີ່ສຸດ. ປັບປຸງຈາກຂໍ້ມູນຈິງ ບໍ່ແມ່ນຄວາມຮູ້ສຶກ." },
            ],
            highlights: ["ບໍ່ມີແຜນ = ບໍ່ເຂົ້າ.", "ຈົດທຸກໄມ້ + ທົບທວນທຸກອາທິດ.", "ປັບປຸງຈາກຕົວເລກຈິງ."],
            summary: "ແຜນ + journal ປ່ຽນການເທຣດຈາກການພະນັນ ເປັນທຸລະກິດທີ່ວັດຜົນແລະພັດທະນາໄດ້. ນີ້ຄືເສັ້ນທາງສູ່ຄວາມສະໝ່ຳສະເໝີ.",
        },
        th: {
    ...
    },
    {
        ...
        lo: {
            ...
            minutes: 8,
            intro: "ລວມທຸກຢ່າງເຂົ້າດ້ວຍກັນເປັນລະບົບເຂົ້າອໍເດີ້ທອງແບບ ສະໄນເປີ້ — ເຂົ້ານ້ອຍ ແຕ່ແມ່ນແລະ RR ສູງ.",
            sections: [
                { h: "1. ກອງ bias (HTF)", p: "H4/H1 ເບິ່ງທິດຫຼັກ + premium/discount. ຈະເລັງ Buy ສະເພາະ discount, Sell ສະເພາະ premium ເທົ່ານັ້ນ ຕັດໄມ້ສວນຖິ້ມໝົດ." },
                { h: "2. ລໍ Liquidity Sweep", p: "ລໍໃຫ້ລາຄາກວາດ high/low ສຳຄັນ (stop hunt) ກ່ອນ. ຢ່າເຂົ້າກ່ອນ sweep ເດັດຂາດ — ນີ້ຄືກັບດັກທີ່ດັກລາຍຍ່ອຍ." },
                { h: "3. ຢືນຢັນ LTF", p: "M5/M1 ລໍ CHoCH/BOS + ແທ່ງປິດຢືນຢັນ ເຂົ້າທີ່ OB/FVG ນ້ອຍ ເຮັດໃຫ້ SL ສັ້ນຫຼາຍ (ຈຸດທີ່ຜິດແຜນຊັດເຈນ)." },
                { h: "4. ຕັ້ງ TP ເປັນຊັ້ນ", p: "TP1 = liquidity ໃກ້ສຸດ (ປິດບາງສ່ວນ + ເລື່ອນ SL break-even), TP2 ກາງ, TP3 ໄກ. RR ລວມຄວນໄດ້ 1:3 ຂຶ້ນໄປ." },
            ],
            highlights: ["ເຂົ້າສະເພາະ discount(Buy)/premium(Sell).", "ຫ້າມເຂົ້າກ່ອນ sweep.", "ຢືນຢັນ LTF ກ່ອນ → SL ສັ້ນ.", "TP ເປັນຊັ້ນ RR 1:3+."],
            summary: "Sniper entry = ອົດທົນລໍຈຸດ A+ ທີ່ທຸກຢ່າງຕົງກັນ ແລ້ວເຂົ້າແມ່ນໆ ຄັ້ງດຽວ. ຄຸນນະພາບຊະນະປະລິມານສະເໝີ.",
        },
        th: {
...
const T = {
    lo: {
        ...
        howToIntro: "ເພື່ອໃຫ້ AI ວິເຄາະຖືກຕ້ອງ ກະລຸນາຖ່າຍຮູບ (screenshot) ກຣາຟຕາມນີ້:",
        ...
        howTo3Desc: "ໃຫ້ເຫັນແທ່ງທຽນຫຼ້າສຸດ ແລະ ລາຄາປັດຈຸບັນ. ຮູບຊັດ ບໍ່ມົວ ບໍ່ຕັດຂອບ. ໃຊ້ XAU/USD (ຄຳ) ເປັນຫຼັກ.",
        howToTip: "ເຄັດລັບ: ຮູບຍິ່ງຊັດ ແລະ ຂໍ້ມູນຍິ່ງຄົບ AI ຍິ່ງວິເຄາະແມ່ນ.",
        howToClose: "ເຂົ້າໃຈແລ້ວ",
        ...
        freeDesc: "ບົດຮຽນພາຍໃນແອັບ ມີຮູບປະກອບ — ອ່ານຟຣີບາງບົດ, ບົດເລິກໆປົດລັອກດ້ວຍ VIP.",
        ...
        ob4Desc: "ເປີດບັນຊີເທຣດຜ່ານ KVB ໃນແອັບ → ຮັບເງິນຄືນ 15$ ຕໍ່ 1 lot ທີ່ເທຣດ. ກົດທີ່ການ໌ດໂບຣກເກີໃນໜ້າຫຼັກ.",
        ...
        devConflictTitle: "ສະມາຊິກນີ້ຖືກໃຊ້ຢູ່ອຸປະກອນອື່ນ", devConflictDesc: "ບັນຊີ VIP/Life Time ນີ້ ຖືກຜູກໄວ້ກັບອຸປະກອນເຄື່ອງເກົ່າແລ້ວ. 1 ສະມາຊິກ ໃຊ້ໄດ້ 1 ເຄື່ອງເທົ່ານັ້ນ. ຖ້າທ່ານປ່ຽນເຄື່ອງ ກະລຸນາຕິດຕໍ່ Admin ເພື່ອຍ້າຍສິດໄປເຄື່ອງໃໝ່.", devConflictWa: "ຕິດຕໍ່ Admin ເພື່ອຍ້າຍເຄື່ອງ", devConflictTrial: "ໃຊ້ແບບທົດລອງຕໍ່ໄປ",
        ...
        rebatePromoTitle: "🎁 ຮັບສ່ວນຫຼຸດ SniperTech AI 50%", rebatePromoDesc: "ເມື່ອເທຣດຜ່ານບັນຊີນີ້ — ສະໝັກ VIP ຄັ້ງຕໍ່ໄປ ຮັບສ່ວນຫຼຸດທັນທີ 50%",
    ...
    th: {
        ...
        ob4Desc: "เปิดบัญชีเทรดผ่าน KVB ในแอป → รับเงินคืน 15$ ต่อ 1 lot ที่เทรด กดที่การ์ดโบรกเกอร์ในหน้าหลัก",
        ...
        devConflictTitle: "สมาชิกนี้ถูกใช้อยู่บนอุปกรณ์อื่น", devConflictDesc: "บัญชี VIP/Life Time นี้ถูกผูกไว้กับเครื่องเดิมแล้ว 1 สมาชิกใช้ได้ 1 เครื่องเท่านั้น หากคุณเปลี่ยนเครื่อง กรุณาติดต่อ Admin เพื่อย้ายสิทธิ์มาเครื่องใหม่", devConflictWa: "ติดต่อ Admin เพื่อย้ายเครื่อง", devConflictTrial: "ใช้แบบทดลองต่อไป",
        ...
        rebatePromoTitle: "🎁 รับส่วนลด SniperTech AI 50%", rebatePromoDesc: "เมื่อเทรดผ่านบัญชีนี้ — สมัคร VIP ครั้งต่อไป รับส่วนลดทันที 50%",
    ...
    en: {
        ...
        ob4Desc: "Open a trading account via KVB in-app → earn 15$ per lot traded. Tap a broker card on Home.",
        ...
        devConflictTitle: "This membership is active on another device", devConflictDesc: "This VIP/Life Time account is already bound to another device. One membership works on one device only. If you changed devices, contact Admin to move your access to this new device.", devConflictWa: "Contact Admin to move device", devConflictTrial: "Keep using trial",
        ...
        rebatePromoTitle: "🎁 Get 50% off SniperTech AI", rebatePromoDesc: "Trade through this account — get 50% off your next VIP subscription",
...
function SniperTechX() {
    ...
    // When a paid membership is already bound to another device, we don't grant
    // VIP on this device — we surface this so the user can ask admin to re-bind.
    ...
    // ── Re-verify device binding on app load ──
    // A returning device that was un-bound (membership moved elsewhere by admin)
    // must not keep a stale VIP from localStorage. Check the cloud once on mount.
    React.useEffect(() => {
        if (!user || !user.email || isAdminEmail(user.email)) return;
        const vip = user.plan && user.plan !== "Trial";
        if (!vip) return;
        const myDevice = getDeviceId();
        let cancelled = false;
        getMemberCloud(user.email).then((m) => {
            if (cancelled || !m) return;
            const boundTo = m.deviceId || "";
            // Bound to a DIFFERENT device → revoke VIP on this device.
            if (boundTo && boundTo !== myDevice) {
                const downgraded = { ...user, plan: "Trial", expiresAt: Math.min(user.expiresAt || 0, Date.now()) };
                setUser(downgraded);
                try { localStorage.setItem("sniper_user", JSON.stringify(downgraded)); } catch(e) {}
                setDeviceConflict({ email: user.email, plan: m.plan || "VIP", boundTo });
            }
        }).catch(() => {});
...
Respond ONLY a valid JSON object (no markdown). Text in ${outLang}; digits as digits; each field ONE short line unless stated:
{
  ...
  "quick_map": "in ${outLang} — ONE line action plan"
        ...
        try {
            ...
            catch (netErr) { clearTimeout(timer); throw new Error(netErr.name === "AbortError" ? "ໃຊ້ເວລານານເກີນ — ລອງໃໝ່ ຫຼື ໃຊ້ຮູບໜ້ອຍລົງ." : "ເຊື່ອມຕໍ່ AI ບໍ່ໄດ້ — ກວດ internet."); }
            clearTimeout(timer);
    ...
    const onPaid = (days, plan) => {
        ...
        setUser((u) => {
            ...
            // Activating here binds the membership to THIS device.
            if (nu.email) { saveAccount({ email: nu.email, plan: p, expiresAt: newExpiry, name: nu.name }); setMemberCloud(nu.email, p, newExpiry, myDevice); }
            return nu;
        });
    ...
    if (!user)
        return React.createElement(Login, { onLogin: (u) => {
            ...
            if (isAdminEmail(u.email)) setIsAdmin(true);
                ...
                const cloudIsBetter = m.expiresAt > (u.expiresAt || 0);
                const boundTo = m.deviceId || "";
                // ── Device binding: a paid membership lives on ONE device ──
                if (!boundTo) {
                    // Not bound yet → bind to THIS device (first device wins) and grant.
                    if (cloudIsBetter) {
                        ...
                        setMemberCloud(nu.email, nu.plan, nu.expiresAt, myDevice);
                    } else {
                        // Same/older expiry but still claim the binding for this device.
                        setMemberCloud(u.email, u.plan, u.expiresAt, myDevice);
                    }
                } else if (boundTo === myDevice) {
                    // This is the bound device → grant the membership normally.
                    if (cloudIsBetter) {
                        const nu = { ...u, plan: m.plan || "VIP", expiresAt: m.expiresAt };
                        setUser(nu);
                        try { localStorage.setItem("sniper_user", JSON.stringify(nu)); } catch(e) {}
                    ...
                    // Bound to a DIFFERENT device → do NOT grant VIP here.
                    // Keep this device on Trial/expired and flag the conflict so the
    ...
    // ── Device conflict: membership bound to another device ──
    if (deviceConflict) {
        const dcWa = `https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent("📱 ຂໍຍ້າຍເຄື່ອງ VIP / Move device\nEmail: " + deviceConflict.email + "\nPlan: " + deviceConflict.plan + "\nNew device ID: " + getDeviceId())}`;
        return React.createElement("div", { style: { minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "'LaoOverride','Noto Sans Lao','Inter',system-ui,sans-serif", display: "flex", alignItems: "center", justifyContent: "center", padding: "32px 20px" } },
            React.createElement("div", { style: { maxWidth: 420, width: "100%", background: C.panel, border: `1px solid ${C.amber}`, borderRadius: 20, padding: "28px 22px", textAlign: "center", position: "relative", overflow: "hidden" } },
                React.createElement("div", { "aria-hidden": true, style: { position: "absolute", top: -60, left: "50%", transform: "translateX(-50%)", width: 260, height: 160, background: `radial-gradient(closest-side, rgba(255,194,75,.3), transparent)`, filter: "blur(20px)" } }),
                React.createElement("div", { style: { position: "relative" } },
                    React.createElement("div", { style: { fontSize: 48, marginBottom: 12 } }, "📱🔒"),
                    React.createElement("h2", { style: { fontFamily: "'LaoOverride','Sora','Noto Sans Lao',sans-serif", fontSize: 21, fontWeight: 800, margin: 0, lineHeight: 1.25, color: C.amber } }, t("devConflictTitle")),
                    React.createElement("p", { style: { color: C.mut, fontSize: 14, lineHeight: 1.7, marginTop: 14 } }, t("devConflictDesc")),
                    React.createElement("a", { href: dcWa, target: "_blank", rel: "noopener noreferrer", className: "fx-btn", style: { display: "block", marginTop: 20, padding: "14px", borderRadius: 13, border: "none", background: C.wa, color: "#fff", fontSize: 15, fontWeight: 700, cursor: "pointer", textDecoration: "none", fontFamily: "inherit" } }, "💬 " + t("devConflictWa")),
                    React.createElement("button", { onClick: () => setDeviceConflict(null), className: "fx-btn", style: { display: "block", width: "100%", marginTop: 10, padding: "13px", borderRadius: 13, border: `1px solid ${C.line}`, background: "transparent", color: C.mut, fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" } }, t("devConflictTrial")),
                    React.createElement("button", { onClick: () => { setUser(null); setDeviceConflict(null); try { localStorage.removeItem("sniper_user"); } catch(e) {} }, style: { display: "block", width: "100%", marginTop: 14, background: "none", border: "none", color: C.mut, fontSize: 12.5, cursor: "pointer", fontFamily: "inherit" } }, "← " + (t("logout") || "ອອກຈາກລະບົບ")))));
    }
            ...
            nav === "home" && (React.createElement("div", { className: "fx-rise" },
                ...
                React.createElement("div", { style: { marginTop: 26 } },
                    ...
                    React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 12, marginTop: 14 } }, [
                        // KCM temporarily admin-only — hidden from regular users, only KVB shows for them.
                        isAdmin && { logo: KCM_LOGO, alt: "KCM Trade", title: t("rebateTitle"), cta: t("registerKcm"), url: KCM_REGISTER_URL },
                        { logo: KVB_LOGO, alt: "KVB", title: t("rebateTitleKvb"), cta: t("registerKvb"), url: KVB_REGISTER_URL },
                    ].filter(Boolean).map((b, i) => React.createElement("section", { key: i, style: { borderRadius: 18, border: `1px solid ${C.line}`, background: `linear-gradient(180deg,${C.panel},${C.bg2})`, padding: "16px 18px", display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap", position: "relative", overflow: "hidden" } },
                        ...
                        React.createElement("a", { className: "fx-link", href: b.url, target: "_blank", rel: "noopener noreferrer", style: { display: "inline-flex", alignItems: "center", gap: 8, textDecoration: "none", background: `linear-gradient(95deg,${C.blue},${C.blueLt})`, color: "#04101F", fontWeight: 700, fontSize: 13.5, padding: "11px 16px", borderRadius: 11, whiteSpace: "nowrap" } },
                            b.cta, " \u2192")))),
                    React.createElement("section", { style: { marginTop: 14, borderRadius: 18, border: `1px solid ${C.amber}`, background: "linear-gradient(120deg,rgba(255,194,75,.14),rgba(255,107,107,.08))", padding: "16px 18px", display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap", position: "relative", overflow: "hidden" } },
                        React.createElement("span", { "aria-hidden": true, style: { position: "absolute", right: -20, top: -20, width: 90, height: 90, borderRadius: "50%", background: "radial-gradient(circle,rgba(255,194,75,.28),transparent 70%)", pointerEvents: "none" } }),
                        React.createElement("span", { style: { fontSize: 30, lineHeight: 1, flexShrink: 0 } }, "🎁"),
                        React.createElement("div", { style: { flex: 1, minWidth: 200 } },
                            React.createElement("div", { style: { fontFamily: "'LaoOverride','Sora','Noto Sans Lao',sans-serif", fontWeight: 800, fontSize: 15.5, color: C.amber } }, t("rebatePromoTitle")),
                            React.createElement("div", { style: { color: C.text, fontSize: 12.5, lineHeight: 1.6, marginTop: 4, opacity: .9 } }, t("rebatePromoDesc"))))))),
    ...
    return (React.createElement("div", { className: "fx-rise", style: { marginTop: 22, display: "flex", flexDirection: "column", gap: 14 } },
        ...
        data.consensus && (React.createElement("div", { style: { borderRadius: 14, border: `1px solid ${data.consensus.agree ? C.green : C.amber}`, background: data.consensus.agree ? "rgba(63,217,138,.08)" : "rgba(255,194,75,.08)", padding: "12px 15px" } },
            ...
            React.createElement("div", { style: { display: "flex", gap: 8, flexWrap: "wrap" } }, data.consensus.votes.map((v, i) => React.createElement("div", { key: i, style: { display: "flex", alignItems: "center", gap: 5, background: C.panel, border: `1px solid ${C.line}`, borderRadius: 99, padding: "4px 10px", fontSize: 11 } },
                React.createElement("span", null, v.engine === "claude" ? "🟣" : v.engine === "gpt" ? "🟢" : "🔵"),
                React.createElement("span", { style: { fontWeight: 700, color: C.text } }, v.engine === "claude" ? "Claude" : v.engine === "gpt" ? "ChatGPT" : "Gemini"),
                React.createElement("span", { style: { color: v.direction === "Buy" ? C.green : v.direction === "Sell" ? C.red : C.amber, fontWeight: 700 } }, v.direction || "—")))))),
            ...
            React.createElement("div", { style: { background: C.panel, border: `1px solid ${C.line}`, borderRadius: 18, padding: "22px 20px", boxShadow: `0 24px 60px -30px ${C.glow}` } },
                ...
                React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 12 } },
                    ...
                    mode === "signup" && (React.createElement(React.Fragment, null,
                        ...
                        React.createElement(Field, { label: "ລະຫັດຜູ້ແນະນຳ (ຖ້າມີ)", hint: "ໃສ່ລະຫັດຈາກຜູ້ທີ່ແນະນຳທ່ານ" },
                            React.createElement("input", { className: "lg-in", value: refInput, onChange: (e) => setRefInput(e.target.value.toUpperCase()), placeholder: "ເຊັ່ນ: KANHSTARFX" })),
...
// ── Slip Upload Form (real payment submission via WhatsApp) ──────
...
function PaymentScreen({ t, lang, setLang, locked, onPaid, onBack, onLogout }) {
    ...
    const activateCode = async () => {
        ...
        if (ACTIVATION_CODES[code]) {
            ...
            setActMsg(okMsg(found.days));
            setTimeout(() => onPaid(found.days, found.plan), 1200);
            return;
        }

        // 2) Google Sheets — primary source of truth (all via GET to avoid CORS)
        if (SHEETS_URL) {
            try {
                // checkCode via GET
        ...
        try {
            ...
            if (!entry) { setActMsg("❌ Code ບໍ່ຖືກຕ້ອງ"); return; }
            ...
            setActMsg(okMsg(entry.days));
            setTimeout(() => onPaid(entry.days, "VIP"), 1200);
        } catch(e) {
            setActMsg("❌ Code ບໍ່ຖືກຕ້ອງ");
        }
    };
...
function NewsRoom({ t, notify: extNotify, setNotify: extSetNotify, isAdmin = false }) {
    ...
    return (React.createElement("section", { style: { marginTop: 18 } },
        React.createElement("div", { style: { background: C.panel, border: `1px solid ${C.line}`, borderRadius: 18, padding: "20px 18px" } },
            ...
            isAdmin && (React.createElement("div", { style: { marginTop: 14, paddingTop: 14, borderTop: `1px solid ${C.line}` } },
                React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 8 } },
                    ...
                    React.createElement("div", { style: { display: "flex", gap: 8 } },
                        React.createElement("button", { onClick: () => imgRef.current && imgRef.current.click(), className: "fx-btn", style: { padding: "9px 13px", borderRadius: 10, border: `1px solid ${C.line}`, background: C.bg2, color: C.mut, fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "inherit", whiteSpace: "nowrap" } }, "\uD83D\uDDBC\uFE0F ແນບຮູບ"),
        ...
        React.createElement("div", { style: { marginTop: 14, display: "flex", flexDirection: "column", gap: 10 } }, posts.length === 0 ? (React.createElement("div", { style: { color: C.mut, fontSize: 13, textAlign: "center", padding: "24px 0" } }, t("roomEmpty"))) : posts.map((p) => (React.createElement("div", { key: p.id, className: "fx-card", style: { borderRadius: 14, border: `1px solid ${C.line}`, background: C.panel, padding: "14px 16px" } },
            ...
            isAdmin && React.createElement("button", { onClick: () => setPosts(prev=>prev.filter(x=>x.id!==p.id)), style: { marginTop: 6, background: "none", border: "none", color: C.mut, fontSize: 11, cursor: "pointer", fontFamily: "inherit" } }, "\uD83D\uDDD1 ລຶບ"))))),
...
// ── Admin API Key Renewal Reminder ───────────────────────────
function AdminApiReminder() {
    // Hardcoded reminder dates — update these when you renew keys
    const today = new Date();
    const reminders = [
        { name: "Anthropic (Claude)", emoji: "🟣", renewUrl: "https://console.anthropic.com", note: "ກວດ balance ຫຼື top-up credit", dueDate: new Date(today.getFullYear(), today.getMonth() + 1, 1) },
        { name: "OpenAI (ChatGPT)", emoji: "🟢", renewUrl: "https://platform.openai.com/settings/billing", note: "ກວດ billing ຫຼື top-up credit", dueDate: new Date(today.getFullYear(), today.getMonth() + 1, 1) },
        { name: "Google (Gemini)", emoji: "🔵", renewUrl: "https://aistudio.google.com/app/apikey", note: "ຟຣີ — ກວດ quota ລາຍວັນ", dueDate: new Date(today.getFullYear(), today.getMonth() + 2, 1) },
    ];
    ...
    return React.createElement("div", { style: { marginTop: 14, borderRadius: 16, border: `1px solid ${C.amber}`, background: "rgba(255,194,75,.08)", padding: "14px 16px" } },
        React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8, marginBottom: 10 } },
            React.createElement("span", { style: { fontSize: 18 } }, "⏰"),
            React.createElement("span", { style: { fontWeight: 700, fontSize: 14, color: C.amber } }, "ແຈ້ງເຕືອນຕໍ່ອາຍຸ API Key")),
        urgent.map((r, i) => {
            ...
            return React.createElement("div", { key: i, style: { display: "flex", alignItems: "center", gap: 10, padding: "9px 11px", borderRadius: 10, background: "rgba(255,194,75,.06)", border: `1px solid rgba(255,194,75,.25)`, marginBottom: 6 } },
                ...
                React.createElement("div", { style: { textAlign: "right" } },
                    React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: diff <= 0 ? C.red : C.amber } }, diff <= 0 ? "⚠️ ໝົດແລ້ວ!" : `ອີກ ${diff} ມື້`),
                    React.createElement("a", { href: r.renewUrl, target: "_blank", rel: "noopener noreferrer", style: { fontSize: 10, color: C.blueLt, textDecoration: "none", fontWeight: 600 } }, "ຈ່າຍ →")));
        }));
}
...
function VipCodeGenerator({ t }) {
    ...
    const generateCode = () => {
        ...
        // Save to Google Sheets via GET (avoids CORS — all params in URL)
        if (SHEETS_URL) {
    ...
    return React.createElement("div", { className: "fx-rise" },
        ...
        React.createElement("section", { style: { ...card, marginBottom: 14, background: "linear-gradient(135deg,rgba(38,130,255,.12),rgba(63,217,138,.06))" } },
            ...
            React.createElement("div", { style: { position: "relative" } },
                ...
                React.createElement("div", { style: { fontSize: 12.5, color: C.cyan, marginTop: 4, lineHeight: 1.6 } }, "ວາງແຜນຜ່ານກອງທຶນ Funded · Prop Firm · Futures — ເນັ້ນ ", React.createElement("b", { style: { color: C.green } }, "ບໍ່ໃຫ້ໂດນ SL ຈົນສອບຕົກ"), " ປົກປ້ອງທຶນກ່ອນ"))),
        ...
        React.createElement("section", { style: card },
            React.createElement("label", { style: { ...lbl, marginBottom: 10, fontSize: 12.5, color: C.text, fontWeight: 700 } }, "📊 ອັບໂຫຼດກຣາຟ XAU/USD"),
            charts.length === 0
...
function FundedResult({ data, fmt }) {
    if (data && data.readable === false)
        return React.createElement("section", { style: { marginTop: 14, background: C.panel, border: `1px solid ${C.red}`, borderRadius: 16, padding: 18, color: "#FFC4C4" } }, data.note || "ອ່ານກຣາຟບໍ່ໄດ້ — ໃຊ້ຮູບຊັດກວ່າ.");
    const s = data.setup || {};
    // Normalise direction → one of Buy / Sell / Wait
    const dirRaw = String(s.direction || "").toLowerCase();
    const isBuy = dirRaw.includes("buy") || dirRaw.includes("ຊື້") || dirRaw.includes("ซื้อ");
    const isSell = dirRaw.includes("sell") || dirRaw.includes("ຂາຍ") || dirRaw.includes("ขาย");
    const dirKind = isBuy ? "buy" : isSell ? "sell" : "wait";
    const dirColor = dirKind === "buy" ? C.green : dirKind === "sell" ? C.red : C.amber;
    const dirLabel = dirKind === "buy" ? "BUY · ຊື້" : dirKind === "sell" ? "SELL · ຂາຍ" : "WAIT · ລໍຖ້າ";
    const dirIcon = dirKind === "buy" ? "🟢 ▲" : dirKind === "sell" ? "🔴 ▼" : "🟡 ⏸";
    const meterColor = data.risk_meter === "ປອດໄພ" ? C.green : data.risk_meter === "ສ່ຽງ" ? C.red : C.amber;

    // Grouped card with a clear heading — easier to scan than one long stack
    const sectionCard = (icon, title, color, children) => React.createElement("div", { style: { background: C.bg2, border: `1px solid ${C.line}`, borderRadius: 14, overflow: "hidden" } },
        ...
        React.createElement("div", { style: { position: "relative", display: "flex", flexDirection: "column", gap: 12 } },
            ...
            React.createElement("div", { style: { textAlign: "center", padding: "16px 12px", borderRadius: 16, border: `2px solid ${dirColor}`, background: dirColor + "14" } },
                ...
                React.createElement("div", { style: { display: "flex", gap: 8, justifyContent: "center", flexWrap: "wrap", marginTop: 10 } },
                    s.confidence && React.createElement("span", { style: { fontSize: 11.5, fontWeight: 700, color: C.cyan, border: `1px solid ${C.line}`, borderRadius: 99, padding: "3px 11px", background: C.bg } }, "ໝັ້ນໃຈ " + s.confidence),
                    s.rr && React.createElement("span", { style: { fontSize: 11.5, fontWeight: 700, color: C.blueLt, border: `1px solid ${C.line}`, borderRadius: 99, padding: "3px 11px", background: C.bg } }, "RR " + s.rr),
                    data.risk_meter && React.createElement("span", { style: { fontSize: 11.5, fontWeight: 700, color: meterColor, border: `1px solid ${meterColor}`, borderRadius: 99, padding: "3px 11px", background: meterColor + "12" } }, "ຄວາມສ່ຽງ " + data.risk_meter))),

            // ── HTF bias banner: explains WHY this direction (prevents the "Buy vs Sell" confusion) ──
            data.htf_bias && React.createElement("div", { style: { display: "flex", gap: 9, alignItems: "flex-start", background: "rgba(38,130,255,.08)", border: `1px solid ${C.blue}`, borderRadius: 12, padding: "10px 13px" } },
                React.createElement("span", { style: { fontSize: 15, lineHeight: 1.4 } }, "🧭"),
                React.createElement("div", null,
                    React.createElement("div", { style: { fontSize: 10.5, color: C.mut, fontWeight: 700, marginBottom: 2 } }, "ທິດທາງຫຼັກ (HTF Bias)"),
                    React.createElement("div", { style: { fontSize: 13, color: C.text, lineHeight: 1.6 } }, data.htf_bias))),

            data.challenge_summary && React.createElement("div", { style: { fontSize: 11.5, color: C.mut, lineHeight: 1.6, textAlign: "center" } }, data.challenge_summary),
            ...
            data.daily_plan && sectionCard("📅", "ແຜນປະຈຳວັນ", C.blueLt, data.daily_plan),
            data.pass_pace && sectionCard("🏁", "ຈັງຫວະຜ່ານກອງທຶນ", C.green, data.pass_pace),
            data.dll_guard && sectionCard("⚠️", "ກັນ DLL ແຕກ (ຫ້າມເຮັດ)", C.amber, data.dll_guard),

            data.quick_map && React.createElement("div", { style: { background: "linear-gradient(95deg,rgba(38,130,255,.16),rgba(63,217,138,.10))", border: `1px solid ${C.blue}`, borderRadius: 12, padding: "13px 14px", fontSize: 13.5, fontWeight: 700, color: C.cyan, textAlign: "center" } }, "🎯 " + data.quick_map),
            React.createElement("div", { style: { fontSize: 10.5, color: C.mut, textAlign: "center", marginTop: 2, lineHeight: 1.5 } }, "⚠️ ນີ້ແມ່ນການວິເຄາະຊ່ວຍຕັດສິນໃຈ ບໍ່ແມ່ນຄຳແນະນຳການລົງທຶນ — ບໍລິຫານຄວາມສ່ຽງເອງສະເໝີ")
        ));
}
...
function AIEnginePanel({ t, engines, setEngines }) {
    const AIS = [
        { id: "claude", name: "Claude", icon: "🟣", desc: t("aiClaudeDesc") },
