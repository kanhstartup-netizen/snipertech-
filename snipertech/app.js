...
OUTPUT: respond ONLY with valid JSON (no markdown, no extra text). Be concise — short values only.`;
        ...
        try {
            ...
            if (activeEngines.length <= 1) {
                ...
                // Show Claude result IMMEDIATELY — don't wait for others
                setResult({ ...parsed, consensus: { votes: [{ engine: "claude", bias: parsed.bias, direction: parsed.setups?.[0]?.direction }], agree: null, majority: parsed.setups?.[0]?.direction, summary: "⏳ ກຳລັງລວບລວມ AI ອື່ນ..." } });
                setStage("");
                // Run GPT + Gemini in PARALLEL (not sequential)
                const otherEngines = activeEngines.filter(e => e !== "claude");
                const maxTok2 = 1200; // ⚡ shorter = faster consensus
                const otherResults = await Promise.allSettled(otherEngines.map(async (engine) => {
                    try {
                        const ctrl2 = new AbortController();
                        setTimeout(() => ctrl2.abort(), 45000); // 45s max per engine
                        const rb = engine === "gpt"
